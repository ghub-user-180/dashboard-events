...
- 2026-05-28 (Design-Loop nach Atelier-Pattern): Erste vollstaendige Iteration im Genre [`stille-uebersicht`](../../Tools/Design-Atelier/genres/stille-uebersicht.md). Eigene [`design.md`](design.md) angelegt, Loop ueber sechs Aenderungen gefahren. (1) Tabelle responsive mit Prioritaets-Spalten: drei Breakpoints, <700px nur Datum / Titel / Ort / Marker, 700-1100px ohne Lokalitaet/Land, >1100px alle Spalten. (2) Filter-Leiste mit einklappbarer Sekundaer-Sektion unter <700px (Datum bleibt sichtbar, Rest hinter «Mehr Filter»-Button mit Aktive-Filter-Count). (3) Beschreibungs-Spalte zu Kategorie-Spalte umfunktioniert: Badge in eigener Spalte (Titel buendig links), Beschreibung wandert in Tooltip am Titel, Indikator-Glyph `ⓘ` zeigt wo Tooltip verfuegbar. (4) Filter visuell gruppiert ueber Whitespace (Datum / Kategorie / Dauer / Geo-Cluster / Quelle / Toggles / Reset), Reset als Text-Link statt dashed Pill. (5) Toggle-Markierungen («Nur Favoriten» / «Ignorierte») als randlose, semantisch farbcodierte Pills (Gruen/Rot) — klar abgesetzt von den gerahmten Metadaten-Filter-Pills. (6) Aktualisieren-Button durch dezentes Lucide-Refresh-Icon neben dem Titel ersetzt (Rotation beim Laden), Stand-Indikator als Subtitel unter «Events» statt rechts in Aktions-Zone. Nav-Jumps («Nach oben» / «Nach unten») als Text-Links rechtsbuendig vereinheitlicht. Validierungs-Rueckspiel ins Atelier: neuer Skill `quiet-controls` aus dem Loop heraus geschrieben.
